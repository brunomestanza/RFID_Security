...
<html>
	<head>
		<title>RFID LOGIN</title> <!--Titulo da aba-->
		<link rel="stylesheet" href="_styles/style-user-login.css" type="text/css" media="all"/> <!--Definicao do estilo do site-->
	</head>
	<body id = "corpo">
		<div id = "container">
			<div id = "divisao"> <!--Divisao que recebe toda a página-->
				...
				<img id="logo" src="_images/logoteste.png"/> <!--Inserção da imagem-->
				</form>
...
<?php
	if(isset($_POST["button"])){
		...
		if($user == "" OR $pass == ""){
			echo "<script> alert('Preencha todos os campos'); location.href='user-login.php'</script>"; //Erro se o usuário deixar a senha ou o login sem preencher
		}	
		...
		if ($row > 0){
			...
			if ($dadosusuario["Permissao"] == 1 OR $dadosusuario["Permissao"] == 2){
				echo "<script> alert('Bem vindo ao Log de Acessos'); location.href='teste.php'</script>"; //Simulção de usuário administrativo com permissão
				session_start();
				$_SESSION["Usuario"] = $user;
			}
			else{
				echo "<script> alert('Você não possui permissão'); location.href='user-login.php'</script>";
			}
		}
		else{
			echo "<script> alert('Usuario ou Senha Incorretos'); location.href='user-login.php'</script>"; 
		}
	}
?>